...
THT_sti <- read_csv(file="phase01_sinica_corpus/THT_nouns_check.csv",na = "NA")%>%filter(STUDY01 == "V") %>% select(WORD,POS,FREQ,RANK,`條`,`枝`,`根`,`張`,`面`,`片`,`個`,`顆`,`粒`)
THT_alternative <- read_csv(file="phase01_sinica_corpus/THT_nouns_check.csv",na = "NA")%>%filter(STUDY01 == "V") %>% select(WORD,POS,FREQ,RANK,瓣,點,頂,朵,封,幅,節,具,卷,管,面,葉,幀,尊)
knitr::kable(THT_sti)
```
...
## list file names
termfiles <- list.files(path = "phase01_sinica_corpus/classifiers_pool/",pattern = "*_terms.txt$")

## import the terms as the data frame
counts_table <- data.frame(classifier = NULL, N = NULL)

for(termfile in termfiles){
classifier_tmp <- read_lines(file=paste0("phase01_sinica_corpus/classifiers_pool/",termfile) )%>% str_split(pattern = ",") %>% lapply(as.data.frame.list,col.names=c("WORD","N")) %>% bind_rows()
...
for(selected_terms in selected_files){
    terms_tmp <- read_lines(file=paste0("phase01_sinica_corpus/classifiers_pool/",selected_terms) )%>% str_split(pattern = ",") %>% lapply(as.data.frame.list,col.names=c("WORD","N")) %>% bind_rows() %>% filter(!(WORD %in% THT_sti$WORD))

write_csv(terms_tmp,file = paste0("phase01_sinica_corpus/classifiers_pool/stipool_", (str_split(selected_terms,"_") %>% unlist())[2],".csv"))
}
...
stipools <- list.files(path = "phase01_sinica_corpus/classifiers_pool/",pattern = "stipool_[a-z]+_mark.csv")
...
for(i in 1:length(stipools)){
    ...
    assign((str_split(stipools[i],"_") %>% unlist())[2],
           paste((read_csv(paste0("phase01_sinica_corpus/classifiers_pool/",stipools[i]) ) %>% filter(!is.na(In)) %T>% write_csv(file=paste0("phase01_sinica_corpus/classifiers_pool/sti_",(str_split(stipools[i],"_") %>% unlist())[2],".csv") ) %>% pull(WORD)), collapse = "," ) )
    )
}
colnames(sti_table) <- c("幅","具","塊","枚","座")

write_csv(as_tibble(sti_table),file = "phase01_sinica_corpus/classifiers_pool/sti_table.csv")

as_tibble(sti_table)
```

以上表格儲存於["sti_table.csv"](phase01_sinica_corpus/classifiers_pool/sti_table.csv)之內。